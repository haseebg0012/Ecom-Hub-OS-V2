...
export const FinanceProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const getInitialDemoInvoices = (_bizId: string): { invoices: Invoice[]; items: InvoiceItem[] } => {
    return { invoices: [], items: [] };
  };

  const getInitialDemoPayments = (_bizId: string): Payment[] => [];

  const getInitialDemoExpenses = (_bizId: string): Expense[] => [];

  const getInitialDemoInvestments = (_bizId: string): Investment[] => [];
