-- ==============================================================================
-- EcomHub OS — Business Roles & Task Builder Repair & Finalization Migration
-- Product: EcomHub OS | Tagline: Your Business, One Hub.
-- Description: Self-contained robust schema with inline tenant RLS policies
-- ==============================================================================
...
-- Safely drop existing policies if any were partially created
DROP POLICY IF EXISTS "business_role_templates_select_all" ON public.business_role_templates;
DROP POLICY IF EXISTS "business_role_templates_admin_all" ON public.business_role_templates;
DROP POLICY IF EXISTS "workspace_assigned_roles_select_policy" ON public.workspace_assigned_roles;
DROP POLICY IF EXISTS "workspace_assigned_roles_admin_all" ON public.workspace_assigned_roles;
...
DROP POLICY IF EXISTS "role_task_templates_select_all" ON public.role_task_templates;
DROP POLICY IF EXISTS "role_task_templates_admin_all" ON public.role_task_templates;

-- Robust RLS Policies (Using inline EXISTS membership check to avoid external helper dependencies)

-- Global business role templates: readable by all authenticated users, managed by admin/server
...
-- Workspace assigned roles: tenant isolated strictly to members of the respective business
CREATE POLICY "workspace_assigned_roles_select_policy"
ON public.workspace_assigned_roles FOR SELECT
USING (
    EXISTS (
        SELECT 1 FROM public.business_members bm
        WHERE bm.business_id = workspace_assigned_roles.business_id
...
-- Role task templates: readable by all authenticated users
